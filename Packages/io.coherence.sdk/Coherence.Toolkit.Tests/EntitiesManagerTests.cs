...
namespace Coherence.Toolkit.Tests
{
    ...
    public class EntitiesManagerTests : CoherenceTest
    {
        ...
        public async Task Test_CreateClientConnectionEntity_NoPrefab(bool isMyConnection)
        {
            ...
            // Act
            var builder = MockNetworkedEntityBuilder.For(entityA)
                .Comps(comps)
                .ClientID(client0);

            if (isMyConnection)
            {
                _ = builder.SetHasStateAuthority();
            }

            _ = builder.Create(mockClient);
        ...
        public void Test_OnEntityUpdated_AppliesConnectedEntityChanges()
        {
            ...
            MockNetworkedEntityBuilder.For(entityA).Update(mockClient);
            ...
            ClientID? clientID = null, string uuid = null)
        {
            ...
            return MockNetworkedEntityBuilder.For(entityID)
                .Comps(comps)
                .Prefab(mockPrefab)
                .Parent(parent)
                .ClientID(clientID)
                .UUID(uuid)
                .CreateWithResult(mockClient);
        }
