...
namespace Coherence.Core.Tests
{
    ...
    public class OutConnectionTests : CoherenceTest
    {
        ...
        public void Update_WhenChannelHasChange_ShouldSendAPacket()
        {
            ...
            _ = channelMock.Setup(c => c.HasChanges(It.IsAny<IReadOnlyCollection<Entity>>())).Returns(true);
            _ = channelMock.Setup(c => c.Serialize(It.IsAny<SerializerContext<IOutBitStream>>(), simulationFrame, false, ackedEntities)).Returns(true);
        ...
        public void Update_WhenChannelHasChange_ShouldSerializeCorrectChannelID(ChannelID channelID)
        {
            ...
            else
            {
                ...
                _ = newChannelMock.Setup(c => c.HasChanges(It.IsAny<IReadOnlyCollection<Entity>>())).Returns(true);
            }
        ...
        {
            // Arrange
            var entityA = new Entity((Entities.Index)1, 0, Entity.Relative);
        ...
        public void Test_CanSendUpdatesDependsOnHoldCommandsForEntity()
        {
            // Arrange
            var entityA = new Entity((Entities.Index)1, 0, Entity.Relative);
            var entityB = new Entity((Entities.Index)2, 0, Entity.Relative);
            ...
            _ = channelMock.Setup(c => c.HasChangesForEntity(entityA)).Returns(false);
        ...
        public void Test_SentAndAckedChangesClearHeldEntities()
        {
            ...
            _ = channelMock.Setup(c => c.HasChangesForEntity(entityA)).Returns(true);
            ...
            _ = channelMock.Setup(c => c.HasChanges(It.IsAny<IReadOnlyCollection<Entity>>())).Returns(false);
            _ = channelMock.Setup(c => c.HasChangesForEntity(entityA)).Returns(false);
        ...
        public void Test_AddChannel_MustBeValid()
        {
            ...
            _ = Assert.Throws<ArgumentNullException>(() => outConnection.AddChannel((ChannelID)1, null));

            // ChannelID must be < 15 (EndOfChannels)
            _ = Assert.Throws<ArgumentException>(() => AddChannel(ChannelID.EndOfChannels));
            _ = Assert.Throws<ArgumentException>(() => AddChannel((ChannelID)100));
            ...
            _ = Assert.Throws<Exception>(() => AddChannel((ChannelID)1));

            // ChannelID 0 is already used by the default channel
            _ = Assert.Throws<Exception>(() => AddChannel(ChannelID.Default));
        }
        ...
        private Mock<IOutNetworkChannel> CreateChannel()
        {
            ...
            _ = channel.Setup(c => c.HasChanges(It.IsAny<IReadOnlyCollection<Entity>>())).Returns(false);
            _ = channel.Setup(c => c.Serialize(It.IsAny<SerializerContext<IOutBitStream>>(), It.IsAny<AbsoluteSimulationFrame>(), It.IsAny<bool>(), It.IsAny<IReadOnlyCollection<Entity>>()));
            _ = channel.Setup(c => c.MarkAsSent(It.IsAny<uint>())).Returns((Dictionary<Entity, OutgoingEntityUpdate>)null);
        ...
        private void Assert_PacketChannelID(OutPacket outPacket, ChannelID expectedChannelID)
        {
            ...
            var decoded = PacketHeaderReader.ToPacketHeaderInfo(octetStream, basicHeader);
            var bitStream = decoded.Stream;
            _ = Deserialize.ReadFloatingOrigin(bitStream, logger);

            _ = Deserialize.ReadChannelID(bitStream, out var channelID);
