...
namespace Coherence.Editor.Toolkit
{
    ...
    internal partial class CoherenceSyncEditor : BaseEditor
    {
        ...
        private static partial class GUIContents
        {
            ...
            public static readonly GUIContent approveRequests = EditorGUIUtility.TrTextContent("Approve by Default", "When no event listeners are registered, respond to requests with the given value (enable to accept the request, disable to deny it).");
            ...
            public static readonly GUIContent infoOnAuthorityRequest =
                EditorGUIUtility.TrTextContent("When someone has requested authority over this entity");
            ...
            public static readonly GUIContent infoOnAuthorityTransferCompleteProperty =
                EditorGUIUtility.TrTextContent("When the authority transfer is completed");
            ...
            public static readonly GUIContent infoOnNetworkedParentChanged =
                EditorGUIUtility.TrTextContent("When the parent (that is networked via CoherenceSync) changes.");
            ...
            public static readonly GUIContent defaultAuthorityTransferTypeSettings = EditorGUIUtility.TrIconContent("d_Settings", "Open project settings to change default authority transfer type");
        }
        ...
        protected class UnitySyncEvents
        {
            ...
            internal void Init(SerializedObject serializedObject)
            {
                ...
                {
                    new(serializedObject.FindProperty(nameof(CoherenceSync.OnBeforeNetworkedInstantiation)),
                        GUIContents.infoOnBeforeNetworkedInstantiation),
                    new(serializedObject.FindProperty(nameof(CoherenceSync.OnNetworkedInstantiation)),
                        GUIContents.infoOnNetworkedInstantiation),
                    new(serializedObject.FindProperty(nameof(CoherenceSync.OnNetworkedDestruction)),
                        GUIContents.infoOnNetworkedDestruction),
                    new(serializedObject.FindProperty(nameof(CoherenceSync.OnNetworkedParentChanged)),
                        GUIContents.infoOnNetworkedParentChanged),
                    ...
                    new(serializedObject.FindProperty(nameof(CoherenceSync.OnAuthorityRequest)),
                        GUIContents.infoOnAuthorityRequest),
                    new(serializedObject.FindProperty(nameof(CoherenceSync.OnAuthorityRequestRejected)),
                        GUIContents.infoOnAuthorityRequestRejectedProperty),
                    new(serializedObject.FindProperty(nameof(CoherenceSync.OnAuthTransferComplete)),
                        GUIContents.infoOnAuthorityTransferCompleteProperty),
        ...
        private SerializedProperty authorityTransferConfigProperty;
        ...
        protected override void OnEnable()
        {
            ...
            authorityTransferConfigProperty = serializedObject.FindProperty(nameof(CoherenceSync.authorityTransferConfig));
        ...
        protected override void OnDisable()
        {
            ...
            authorityTransferConfigProperty.Dispose();
        ...
        private void DrawUniqueness()
        {
            ...
            EditorGUI.BeginChangeCheck();
            _ = EditorGUILayout.PropertyField(uniquenessTypeProperty, new GUIContent("Uniqueness"));
            if (EditorGUI.EndChangeCheck())
            {
        ...
        private void DrawSimulate()
        {
            ...
            DrawSimulateInternal(rect, sync, simulationTypeProperty, authorityTransferConfigProperty,
                true);
            EditorGUI.EndDisabledGroup();
        }

        internal static void DrawSimulateInternal(Rect rect, CoherenceSync sync,
            SerializedProperty simulationTypeProperty, SerializedProperty authorityTransferConfigProperty, bool useLabel)
        {
            EditorGUI.BeginChangeCheck();
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                if (useInput)
                {
                    authorityTransferConfigProperty.intValue = (int)CoherenceSync.AuthorityTransferConfig.Stealing;
                    ...
                    if (authorityTransferConfigProperty.serializedObject != simulationTypeProperty.serializedObject)
                    {
                        _ = authorityTransferConfigProperty.serializedObject.ApplyModifiedProperties();
                    }
        ...
        private void DrawLifetime()
        {
            ...
            var rect = EditorGUILayout.GetControlRect();
            DrawLifetimeInternal(rect, lifetimeTypeProperty, authorityTransferConfigProperty, true);
            ...
            SerializedProperty authorityTransferProperty, bool useLabel)
        {
            ...
            if (change.changed)
            {
                ...
                var authorityTransferType = ((CoherenceSync.AuthorityTransferConfig)authorityTransferProperty.intValue).MapToType();

                if ((CoherenceSync.LifetimeType)lifetimeProperty.intValue == CoherenceSync.LifetimeType.Persistent
                    && authorityTransferType == CoherenceSync.AuthorityTransferType.NotTransferable)
                {
        ...
        private void DrawTransferAuthority()
        {
            ...
            {
                var rect = EditorGUILayout.GetControlRect();
                DrawTransferAuthorityInternal(rect,
                    authorityTransferConfigProperty,
                    sync.lifetimeType == CoherenceSync.LifetimeType.Persistent,
                    useLabel: true);

                var authorityTransferValue = (CoherenceSync.AuthorityTransferConfig)authorityTransferConfigProperty.intValue;
                if (authorityTransferValue == CoherenceSync.AuthorityTransferConfig.Default &&
                    GUILayout.Button(GUIContents.defaultAuthorityTransferTypeSettings, GUILayout.ExpandWidth(false), GUILayout.Height(EditorGUIUtility.singleLineHeight)))
                {
            ...
            {
                EditorGUI.indentLevel++;
                if (sync.AuthorityTransfer == CoherenceSync.AuthorityTransferType.Request)
                {
        ...
        public static void DrawTransferAuthorityInternal(Rect rect, SerializedProperty authorityTransferConfigProperty, bool isLifetimePersistent, bool useLabel)
        {
            using var change = new EditorGUI.ChangeCheckScope();

            var previousValue = authorityTransferConfigProperty.intValue;

            var label = useLabel ? new GUIContent("Authority Transfer", authorityTransferConfigProperty.tooltip) : GUIContent.none;

            EditorGUI.BeginChangeCheck();
            {
                var defaultOption = new[] { (CoherenceSync.AuthorityTransferConfig.Default, GetAuthorityTransferDisplayName(CoherenceSync.AuthorityTransferConfig.Default)) };
                var otherOptions = ((CoherenceSync.AuthorityTransferConfig[])Enum.GetValues(typeof(CoherenceSync.AuthorityTransferConfig)))
                        .Where(e => e != CoherenceSync.AuthorityTransferConfig.Default)
                        .Select(e => (e, GetAuthorityTransferDisplayName(e)));

                UIUtils.DrawCustomEnumPopup(rect, label, authorityTransferConfigProperty, defaultOption.Concat(otherOptions).ToArray());
            }
            if (EditorGUI.EndChangeCheck())
            {
                var authorityTransferConfig = (CoherenceSync.AuthorityTransferConfig)authorityTransferConfigProperty.intValue;
                ReportAnalytic("authority_transfer_type", authorityTransferConfig);
            }

            try
            {
                var authorityTransferType = ((CoherenceSync.AuthorityTransferConfig)authorityTransferConfigProperty.intValue).MapToType();

                if (change.changed && isLifetimePersistent && authorityTransferType == CoherenceSync.AuthorityTransferType.NotTransferable)
                {
                    logger.Info("Cannot set transfer authority to NotTransferable, since it's Lifetime is set to Persistent.");
                    authorityTransferConfigProperty.intValue = previousValue;
                }
            }
            catch (Exception e)
            {
        ...
        public static string GetAuthorityTransferDisplayName(CoherenceSync.AuthorityTransferConfig config)
        {
            if (config == CoherenceSync.AuthorityTransferConfig.Default)
            {
                var defaultAuthorityTransferType =
                   UIUtils.GetEnumLabel((CoherenceSync.AuthorityTransferType)RuntimeSettings.Instance.defaultAuthorityTransferType);

                return $"{UIUtils.GetEnumLabel(config)}: {defaultAuthorityTransferType}";
            }

            return UIUtils.GetEnumLabel(config);
        }
