...
namespace Coherence.Editor
{
    ...
    internal class Endpoints
    {
        ...
        public static string Get(string path) => Get(path, ProjectSettings.instance.GetActiveProjectId());

        public static string Get(string path, string projectId)
        {
            var organizationId = ProjectSettings.instance.OrganizationId;
            if (!TryGet(path, organizationId, projectId, out var result, out var error))
            {
