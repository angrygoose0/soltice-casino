...
namespace Coherence.Editor
{
    ...
    internal class BindingsTreeViewBindingItem : BindingsTreeViewItem
    {
        ...
        private void ApplyToAllVariants()
        {
            ...
            foreach (var variantSync in variants)
            {
                if (!variantSync)
                {
                    continue;
            ...
            Debug.Log($"Optimizations successfully applied to all {variants.Length} variants.");
        }
        ...
        private IEnumerable<CoherenceSync> GetAllPrefabVariants()
            => PrefabUtils.FindAllVariants(sync, CoherenceSyncConfigRegistry.Instance.Select(x => x.Sync));
    }
}
