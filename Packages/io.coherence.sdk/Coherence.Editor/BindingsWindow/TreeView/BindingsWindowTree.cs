...
namespace Coherence.Editor
{
    ...
    internal class BindingsWindowTree : TreeView
    {
        ...
        {
            var columns = BindingsWindow.StateController.CurrentState.Columns;
            var columnRect = multiColumnHeader.GetColumnRect(GetVisibleColumnIndex());
            var xPos = columnRect.center.x - (treeViewRect.xMax * .5f);
            xPos = Mathf.Clamp(xPos, 0, multiColumnHeader.state.widthOfAllVisibleColumns - treeViewRect.xMax);
            state.scrollPos = new(xPos, state.scrollPos.y);
            ...
            int GetVisibleColumnIndex()
            {
                ...
                {
                    if (!BindingsWindow.TreeHeader.IsColumnVisible(columnIndex))
                    {
                        continue;
                    }
        ...
        protected override void RowGUI(RowGUIArgs args)
        {
            ...
            for (var visibleColumnIndex = 0; visibleColumnIndex < args.GetNumVisibleColumns(); ++visibleColumnIndex)
            {
                ...
                var columnContent = BindingsWindow.StateController.GetColumnContent(columnIndex);
                var lodStep = BindingsWindow.StateController.GetLODIndexFromColumnIndex(columnIndex);
