...
namespace Coherence.Editor
{
    ...
    internal class Postprocessor : AssetPostprocessor
    {
        ...
        private static bool IsPackagesManifest(string assetPath)
        {
            return assetPath == Paths.packageManifestPath;
        }
            ...
            string[] movedAssets, string[] movedFromAssetPaths)
        {
            ...
            {
                Schemas.state = Schemas.SyncState.Unknown;
            ...
            if (Array.Exists(importedAssets, IsPackagesManifest))
            {
                OnPackagesManifestUpdated();
            }
        }

        private static void OnPackagesManifestUpdated() => UIUtils.ClearSamplesCache();
        ...
        internal static void UpdateRuntimeSettings() => UpdateRuntimeSettings(BuildPreprocessor.GetIsDevelopmentMode());

        internal static void UpdateRuntimeSettings(bool isDevelopmentMode)
        {
            var runtimeSettings = RuntimeSettings.Instance;
            Debug.Assert(runtimeSettings);

            runtimeSettings.schemas = GetSchemaAssets();
            runtimeSettings.SchemaID = BakeUtil.SchemaID;

            if (!SimulatorBuildPipeline.IsBuildingSimulator)
            {
                ProjectSettings.instance.UpdateProjectInRuntimeSettings(isDevelopmentMode);
            }
