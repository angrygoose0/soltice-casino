...
namespace Coherence.Editor
{
    ...
    public class PopupPicker<T> : PopupWindowContent where T : class
    {
        ...
        public PopupPicker(T[] items, int maxRows = 10, float width = 200, Func<T, string> display = null, bool sortByName = true)
        {
            ...
            filteredItems = sortByName ? items.OrderBy(displayFunction).ToArray() : items;
        ...
        private void FilterItems(string text)
        {
            ...
            {
                filteredItems = !sortByName ? availableItems : availableItems.OrderBy(displayFunction).ToArray();
                return;
            }

            var enumerator = availableItems
                .Where(item => displayFunction(item).Contains(text, StringComparison.OrdinalIgnoreCase))
                .Take(MaxResults);

            if (sortByName)
            {
                enumerator = enumerator.OrderBy(displayFunction);
            }

            filteredItems = enumerator.ToArray();
        }
