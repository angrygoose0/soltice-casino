...
namespace Coherence.Toolkit
{
    ...
    internal static class ArchetypeMath
    {
        ...
        internal static (int bits, long minRange, long maxRange) GetRangeAndBitsForEnum(Type enumType)
        {
            int bits;
            long min;
            long max;
            ...
            if (isFlags)
            {
                ...
                min = 0;
                bits = (int)Math.Log(maxValue, 2) + 1;
                max = (int)Math.Pow(2, bits) - 1;  // Needs to be the max combined value of all the bits in the flags
            }
            else
            {
                var values = (int[])Enum.GetValues(enumType);
                min = values.Min();
                max = values.Max();
                bits = GetBitsForIntValue(min, max);
            }
