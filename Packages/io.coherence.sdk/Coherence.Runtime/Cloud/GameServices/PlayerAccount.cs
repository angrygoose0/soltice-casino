...
namespace Coherence.Cloud
{
    ...
    public sealed class PlayerAccount : IDisposable, IEquatable<PlayerAccount>
    {
        ...
        /// <summary>
        /// Number of entities that have logged in to this player account.
        /// </summary>
        /// <remarks>
        /// Increased by one every time that a Login operation is performed targeting this Player Account,
        /// and decreased by one every time that <see cref="Dispose"/> or <see cref="DecrementOwnerCount"/> is executed.
        /// Once the count reaches zero, the cloud services associated with the player account are disposed of
        /// and removed from the list of all player accounts.
        /// </remarks>
        private int ownerCount = 1;

        internal int OwnerCount => ownerCount;
        ...
        public CloudService Services
        {
            ...
            internal set
            {
                ...
                if (services is not null)
                {
                    ...
                    services.AuthClient.OnError -= OnLoggingInFailed;
                ...
                if (value is not null)
                {
                    ...
                    value.AuthClient.OnError += OnLoggingInFailed;
                }
...
#if UNITY
            SimulatorUtility.UseSharedCloudCredentials;
#else
            false;
#endif
        ...
        {
            if (loginInfos.Any(x => x.LoginType is LoginType.Guest or LoginType.LegacyGuest))
            {
                IsGuest = true;
                return;
            }

            if (loginInfos.Count is 0 || loginInfos.All(x => x.LoginType is not LoginType.OneTimeCode and not LoginType.SessionToken))
            {
                IsGuest = false;
            }
        }
        ...
        public override string ToString()
        {
            ...
            {
                return AddSuffix(Username);
            }

            return AddSuffix($"Player {id}");
        ...
        private void OnLoggingInFailed(LoginError error) => SetState(State.LoggedOut);

        /// <summary>
        /// Increments the number of owners that this player account has.
        /// </summary>
        /// <remarks>
        /// The player account will be disposed once its owner count reaches zero.
        /// </remarks>
        internal void IncrementOwnerCount() => ownerCount++;

        /// <summary>
        /// Decrements the number of owners that this player account has.
        /// </summary>
        /// <remarks>
        /// If the owner count reaches zero, the PlayerAccount will be disposed.
        /// </remarks>
        internal void DecrementOwnerCount() => Dispose();
        ...
        internal async ValueTask DisposeAsync(bool waitForOngoingOperationsToFinish)
        {
            ...
            DisposeShared();
            if (shouldDisposeCloudService && services is not null)
            {
