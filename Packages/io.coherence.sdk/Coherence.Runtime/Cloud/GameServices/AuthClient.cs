    ...
    public sealed class AuthClient : IAuthClientInternal, IDisposableInternal
    {
        ...
        internal async Task<LoginResult> Login(LoginInfo info, CancellationToken cancellationToken = default)
        {
            ...
            if (PlayerAccount.Find(info) is { } existingPlayerAccount)
            {
                ...
                if (!ReferenceEquals(existingPlayerAccount.Services.AuthClient, this) && !playerAccountOwnerCountIncremented)
                {
                ...
                existingPlayerAccount.Services.AuthClient.OnLogin += OnExistingPlayerAccountLoggedIn;
                existingPlayerAccount.Services.AuthClient.OnError += OnExistingPlayerAccountLogInFailed;
...
#pragma warning restore CS8524
        ...
        private static async Task<LoginResult> HandleLoginCanceled(CancellationToken cancellationToken)
        {
